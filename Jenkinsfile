...
pipeline {
    ...
    stages {
        stage('Clone Source Repo') {
            ...
            steps {
                dir('source') {
                    git url: repoUrl, branch: 'main', credentialsId: 'd6c2b94a-b122-4a3d-a5b8-380f9a2bf7a2'
                }
            }
        }
                ...
                script {
                    ...
                    def jobs = images.collectEntries { img ->
                        ["Build & Push ${img}": {
                            dir('source') {
                                sh """
                                    docker build -t 192.168.22.62:30002/backend/backend/${img}:${imageTag} -f Dockerfile .
                                    docker push 192.168.22.62:30002/backend/backend/${img}:${imageTag}
                                """
                            }
                        }]
                    }
        ...
        stage('Update Manifest Repo & Conditional Apply') {
            ...
            steps {
                dir('manifest') {
                    git url: manifestUrl, branch: 'main', credentialsId: 'd6c2b94a-b122-4a3d-a5b8-380f9a2bf7a2'

                    script {
                        images.each { img ->
                            sh """
                                sed -i 's|image: .*${img}.*|image: 192.168.22.62:30002/backend/backend/${img}:${imageTag}|' backend-deploy.yaml
                        ...
                        // 변경 사항 감지
                        sh 'git diff --quiet backend-deploy.yaml || echo "changed" > .changed'
                        ...
                        if (changed) {
                            ...
                            withCredentials([usernamePassword(
                                ...
                                sh """
                                    ...
                                    git commit -m "Update backend image tag to ${imageTag}" || true
        ...
        stage('ArgoCD Sync') {
            ...
            steps {
                sh """
                    ...
                    -H 'Content-Type: application/json' \\
                    -d '{}' \\
                    http://192.168.22.62:32080/api/v1/applications/${APP_NAME}/sync
                """
            }
        }
        */
